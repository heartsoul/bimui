...
export function getFileExt(name) {
    ...
    const ret = name.split('.');
    const ext = ret.slice(-1);
    return ext;
}

const videos = ['AVI', 'WMV', 'RM', 'RMVB', 'MPEG1', 'MPEG2', 'MPEG4', 'MP4', '3GP', 'ASF', 'SWF', 'VOB', 'DAT', 'MOV', 'M4V', 'FLV', 'F4V', 'MKV', 'MTS', 'TS'];
...
export function isModel(fileName) {
    if (!fileName) {
        return false;
    }
    const ret = fileName.split('.');
    ...
    if (ext.toUpperCase() === 'RVT') {
        return true;
    }
    return false;
}

export function isDrawing(fileName) {
    if (!fileName) {
        return false;
    }
    const ret = fileName.split('.');

...
const UtilsNetwork = {
    current_network_state: getNetworkState(),
};
/**
 * 注册网络的状态变化监听
...
UtilsNetwork.registNetworkStateListener = function (cb_network_state, cb_network_state_change) {
    NetInfo.fetch().done((net_state) => {
        ...
        NetInfo.addEventListener('change',
            (state) => {
                if (cb_network_state) {
                    cb_network_state(state);
                }

                if (cb_network_state_change) {
                    cb_network_state_change(state);
                }
            });
    });
};
...
UtilsNetwork.isNetworkConnected = function (network_state) {
    ...
    const ns = network_state.toUpperCase();
...
UtilsNetwork.setDefaultListener = function () {
    UtilsNetwork.registNetworkStateListener((network_state) => {
        UtilsNetwork.current_network_state = network_state;
    }, null);
};
